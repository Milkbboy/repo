...
namespace ERang.Data
{
    ...
    {
        public int master_Id; // 마스터의 Id 값
        public string nameDesc; // 마스터 캐릭터의 실제 이름
        public MasterType masterType; // 마스터의 타입
        public string cardNameId; // 마스터 캐릭터의 실제 이름 String이 들어간 데이터 Id
        public string cardDescId; // 마스터의 캐릭터 설명을 담은 String이 들어간 데이터 Id
        public string cardShortDescId; // 마스터 간단 설명
        public int masterAi_id; // 마스터의 공격 타입에 대한 Id (근거리, 원거리, 혹은 폭파 등)
        public int hp; // 마스터 캐릭터의 초기 체력 값
        public int maxHp; // 마스터 캐릭터의 최대 체력 값
        public int atk; // 마스터 캐릭터의 초기 공격력 값 (공격력 값이 0인 캐릭터는 공격을 시도하지 않는다)
        public int def; // 마스터 캐릭터의 초기 방어력 값
        public int startMana; // 마스터 캐릭터의 초기 마나 값
        public int maxMana; // 마스터 캐릭터의 최대 마나 값
        public int manaPerTurn; // 턴이 다시 시작 될 때 얻게 되는 마나 초기 값
        public string startCardDeckIds; // 마스터이 처음 스테이지에 진입 할 때 갖게되는 카드의 복수 값
        public int startArtiFact_id; // 마스터이 처음 시작 시 갖고 있는 아티팩트의 id 값
        public string StartAbilityIds; // 마스터이 선천적으로 가지고 있는 특성 id 값
        public int creatureSlots; // 마스터이 소환할 수 있는 크리쳐의 슬롯 개수
        public int satietyGauge; // 마스터의 포만감 게이지 초기 값
        public int maxSatietyGauge; // 마스터의 포만감 게이지 최대 값
        public List<int> startCardIds = new List<int>(); // 마스터이 처음 스테이지에 진입 할 때 갖게되는 카드 id 리스트
        public List<int> startAbilityIds = new List<int>(); // 마스터이 선천적으로 가지고 있는 특성 id 리스트
        ...
        public void Initialize(MasterDataEntity entity)
        {
            ...
            manaPerTurn = entity.RechargeMana;
