...
namespace ERang
{
    public class AbilitySummonGraveDeck : BaseAbility
    {
        ...
        private IEnumerator SummonCardToLocation(CardAbility cardAbility, DeckKind targetDeck)
        {
            ...
            if (cardIds.Count == 0)
            {
                LogAbility($"그레이브 카드 소환 그룹 {summonGroupId}에서 카드 뽑기 실패. 확률을 확인해주세요.", LogType.Error);
                yield break;
            }
            else
            {
                LogAbility($"그레이브 카드 소환 그룹 {summonGroupId}에서 {string.Join(",", cardIds)}");
            }
            ...
            foreach (int cardId in cardIds)
            {
                ...
                LogAbility($"그레이브 카드 소환: {cardName} -> {targetDeck}");
