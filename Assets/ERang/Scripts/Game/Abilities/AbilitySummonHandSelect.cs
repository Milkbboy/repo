...
namespace ERang
{
    public class AbilitySummonHandSelect : BaseAbility
    {
        public override AbilityType AbilityType => AbilityType.SummonHandSelect;
        ...
        protected override IEnumerator ApplyEffect(CardAbility cardAbility, BoardSlot selfSlot, BoardSlot targetSlot)
        {
            ...
            {
                yield return StartCoroutine(HandDeck.Instance.SummonCardToDeck(selectCard.Card.Id, DeckKind.Hand));
                LogAbility($"핸드로 소환 완료: {selectCard.Card.Name}");
            }

            cardSelectObject.OnConfirmClickCoroutine();

            LogAbility("선택 카드 HandDeck 이동 완료");
        }
