...
namespace ERang
{
    ...
    public class CardSystemTest : MonoBehaviour
    {
        ...
        void TestValueSystem()
        {
            ...
            Debug.Log($"초기 값 - HP: {card.State.Hp}, ATK: {card.State.Atk}, DEF: {card.State.Def}");

            // 값 수정
            card.SetHp(card.State.Hp - 20);
            card.SetAttack(card.State.Atk + 5);

            Debug.Log($"수정 후 - HP: {card.State.Hp}, ATK: {card.State.Atk}, DEF: {card.State.Def}");
        }
        ...
        void TestCardInteractions()
        {
            ...
            // 초기 값 설정
            attackerCard.SetAttack(15);
            defenderCard.SetHp(30);
            defenderCard.SetDefense(5);

            Debug.Log($"공격 전 - 공격자 ATK: {attackerCard.State.Atk}");
            Debug.Log($"공격 전 - 방어자 HP: {defenderCard.State.Hp}, DEF: {defenderCard.State.Def}");

            // 공격 시뮬레이션
            int damage = attackerCard.State.Atk;
            defenderCard.TakeDamage(damage);

            Debug.Log($"공격 후 - 방어자 HP: {defenderCard.State.Hp}, DEF: {defenderCard.State.Def}");

            // 힐링 테스트
            defenderCard.RestoreHealth(10);
            Debug.Log($"힐링 후 - 방어자 HP: {defenderCard.State.Hp}");
        }
        ...
        void TestFeatureToggling()
        {
            ...
            Debug.Log($"골드 카드 데미지 후 HP: {goldCard.State.Hp} (변화 없어야 함)");
