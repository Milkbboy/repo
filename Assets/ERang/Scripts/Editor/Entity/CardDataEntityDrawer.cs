...
namespace ERang
{
    [CustomPropertyDrawer(typeof(CardDataEntity))]
    public class CardDataEntityDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            // Card_Id 값을 가져옵니다.
            SerializedProperty cardIdProperty = property.FindPropertyRelative("Card_Id");
            if (cardIdProperty == null)
            {
                EditorGUI.LabelField(position, "Card_Id: SerializedProperty is null");
                EditorGUI.EndProperty();
                return;
            }
            string cardId = cardIdProperty.intValue.ToString();

            // Card_Id 값을 레이블로 사용합니다.
            label = new GUIContent($"{cardId}");
