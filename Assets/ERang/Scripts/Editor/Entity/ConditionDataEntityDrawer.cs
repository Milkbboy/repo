...
namespace ERang
{
    [CustomPropertyDrawer(typeof(ConditionDataEntity))]
    public class ConditionDataEntityDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            EditorGUI.BeginProperty(position, label, property);

            // ConditionData_Id 값을 가져옵니다.
            SerializedProperty ConditionIdProperty = property.FindPropertyRelative("ConditionData_Id");
            if (ConditionIdProperty == null)
            {
                EditorGUI.LabelField(position, "ConditionData_Id: SerializedProperty is null");
                EditorGUI.EndProperty();
                return;
            }
            string conditionId = ConditionIdProperty.intValue.ToString();

            // ConditionData_Id 값을 레이블로 사용합니다.
            label = new GUIContent($"{conditionId}");
