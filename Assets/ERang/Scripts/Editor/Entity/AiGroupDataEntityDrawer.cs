...
namespace ERang
{
    [CustomPropertyDrawer(typeof(AiGroupDataEntity))]
    public class AiGroupDataEntityDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            EditorGUI.BeginProperty(position, label, property);

            // AiGroupData_Id 값을 가져옵니다.
            SerializedProperty aiGroupIdProperty = property.FindPropertyRelative("AiGroup_Id");
            if (aiGroupIdProperty == null)
            {
                EditorGUI.LabelField(position, "AiGroup_Id: SerializedProperty is null");
                EditorGUI.EndProperty();
                return;
            }
            string aiGroupId = aiGroupIdProperty.intValue.ToString();

            // AiGroup_Id 값을 레이블로 사용합니다.
            label = new GUIContent($"{aiGroupId}");
