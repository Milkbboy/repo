...
namespace ERang
{
    [CustomPropertyDrawer(typeof(AiDataEntity))]
    public class AiDataEntityDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            // Ai_Id 값을 가져옵니다.
            SerializedProperty aiIdProperty = property.FindPropertyRelative("Ai_Id");
            if (aiIdProperty == null)
            {
                EditorGUI.LabelField(position, "Ai_Id: SerializedProperty is null");
                EditorGUI.EndProperty();
                return;
            }
            string aiId = aiIdProperty.intValue.ToString();

            // Ai_Id 값을 레이블로 사용합니다.
            label = new GUIContent($"{aiId}");
