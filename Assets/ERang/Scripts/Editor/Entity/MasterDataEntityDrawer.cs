...
namespace ERang
{
    [CustomPropertyDrawer(typeof(MasterDataEntity))]
    public class MasterDataEntityDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            // Master_Id 값을 가져옵니다.
            SerializedProperty masterIdProperty = property.FindPropertyRelative("Master_Id");
            if (masterIdProperty == null)
            {
                EditorGUI.LabelField(position, "Master_Id: SerializedProperty is null");
                EditorGUI.EndProperty();
                return;
            }
            string masterId = masterIdProperty.intValue.ToString();

            // Master_Id 값을 레이블로 사용합니다.
            label = new GUIContent($"{masterId}");
